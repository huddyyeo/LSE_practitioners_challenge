...
#Loading final data
data2=read.csv("processed_data_training.csv", header = TRUE)
data3=read.csv("processed_data_full.csv", header = TRUE)
...
Yield.spec=ugarchspec(mean.model = list(armaOrder = c(3,1)), variance.model = list(garchOrder = c(1, 2), model = "sGARCH"), distribution.model = "norm")
Inflation.spec=ugarchspec(mean.model = list(armaOrder = c(4,4)), variance.model = list(garchOrder = c(1, 2), model = "sGARCH"), distribution.model = "norm")
GDP.spec=ugarchspec(mean.model = list(armaOrder = c(4,4)), variance.model = list(garchOrder = c(1, 1), model = "sGARCH"), distribution.model = "norm")
RF_rate.spec=ugarchspec(mean.model = list(armaOrder = c(3,3)), variance.model = list(garchOrder = c(1, 2), model = "sGARCH"), distribution.model = "norm")
...
#Fit the DCC model using the DCCspec object and the training data
...
dcc.roll=dccroll(spec=dcc.garch11.spec,data=data3, n.ahead = 1, n.start = 237, refit.every = 1, refit.window = "recursive", window.size = NULL)
dcc.roll #Forecasted models

#Note that the dccroll function re-fits 12 new DCC models on the rolling window of training data, irrespective of the dcc.fit2 object
